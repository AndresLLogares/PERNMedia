...
const Login = () => {
    ...
    const resLink = useSelector(state => state.pink.link)
        ...
        await axios.post(URL + 'login', {
            ...
            .then(async (response) => {
                if (response.data.message !== 'Welcome Back') {
                    ...
                    toast.success(response.data.message + ' ' + response.data.firstname + '!')
                    ...
                    setTimeout(() => window.location.href = 'https://pinkbio.netlify.app/Home', 1000)
                }
            })
    }
                ...
                <form onSubmit={handleSubmitChange} className={styles.form}>
                    ...
                    <Zoom className={styles.zoom} >
                        <div className={styles.sortButton}>
                            <button type='submit' className={styles.button}>Go!<ArrowRightCircle className={styles.iconsFormLog} /></button>
                        </div>
                    </Zoom>
                </form>
                <Zoom className={styles.zoom} >
                    <div className={styles.sortButton}>
                        <button onClick={handlePopUp} className={styles.button}>Forgot your password?</button>
                    </div>
                </Zoom>
            </div>
            {popUp ? null
                ...
                <form onSubmit={handleReset} className={styles.popUp}>
                    ...
                    <Zoom className={styles.zoom} >
                        <div className={styles.sortButton}>
                            <button type='submit' className={styles.button}>Send E-Mail</button>
                        </div>
                    </Zoom>
                </form>
            }
