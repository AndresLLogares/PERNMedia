...
import { GETUSERUUID, GETUSER, GETPOSTBYUSER, GETFRIENDS, GETPOSTBYUUID } from '../actions';
...
const ProfileOut = (props) => {
    ...
    const handleAddContact = async () => {
        await axios.post(URL + 'addfriends', {
            uuid: uuidv4(),
            user: emailUser,
            emailfriend: user?.email,
            namefriend: user?.fullname,
            imagefriend: user?.profilePhoto,
            useruuid: user?.uuid,
        })
            .then((response) => {
    ...
    const handleRemoveContact = async () => {
        let uuid = await friends.filter(item => item.emailfriend === user.email);
            ...
            <div className={styles.sortProfile}>
                    ...
                    <div className={styles.sortProfilePicture}>
                        <Zoom className={styles.zoomProfile} >
                            <div className={styles.imageProfile}>
                                {!profilePhoto ?
                                    <img alt='' src={noProfile} className={styles.backImageSize} />
                                    :
                                    <img alt='' src={profilePhoto} className={styles.backImageSize} />
                                }
                            </div>
                        </Zoom>
                    </div>
                                ...
                                <div className={styles.separateTechno}>
                                    <div className={styles.sortMyTechnologies}>
                                        <div className={styles.sortListTechno}>
                                            {technologiesUser && technologiesUser.map((item, index) => (
                                                <div key={index} className={styles.eachTechno}  >
                                                    <p className={styles.techno}><item.logo className={styles.iconButtonsTech} />{item.name}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                {user.email !== emailUser ?
                    <div className={styles.sortButtonAdd}>
                        {friends && friends.findIndex(item => item.emailfriend === user?.email) >= 0 ?
                            <Zoom className={styles.zoomAdd} >
                                <button onClick={handleRemoveContact} className={styles.buttonFriend} ><RemoveUser className={styles.iconFriend} />Remove Contact</button>
                            </Zoom>
                            :
                            <Zoom className={styles.zoomAdd} >
                                <button onClick={handleAddContact} className={styles.buttonFriend} ><AddUser className={styles.iconFriend} />Add Contact</button>
                            </Zoom>
                            ...
                            {user.jobs && user?.jobs.map((item, index) => (
                                <Fragment key={index} >
                                    <div className={styles.eachEducation} key={index}>
                                        <p className={styles.infoEducation} ><PositionForward className={styles.iconButtonsInfo} />Job Position:{item.jobposition}</p>
                                        <p className={styles.infoEducation}><Work className={styles.iconButtonsInfo} />Employment Type:{item.employmenttype}</p>
                                        <p className={styles.infoEducation}><CalendarDateFill className={styles.iconButtonsInfo} />Start Date:{item.startdate}</p>
                                        <p className={styles.infoEducation}><CalendarDateFill className={styles.iconButtonsInfo} />End Date:{item.enddate}</p>
                                        <p className={styles.infoEducation}><TextDescription className={styles.iconButtonsInfo} />Description:{item.description}</p>
                                        <hr className={styles.hrInfo} />
