...
const Profile = () => {
    ...
    const handleAddTechno = async () => {
        ...
        await axios.post(URL + 'addtechno', {
            technologies: techno,
            email: emailUser
        })
            .then(async (response) => {
    ...
    const HandleImage = async (event) => {
        ...
        axios.post(URL + 'UploadPhotos/back', {
            email: emailUser,
    ...
    const HandleImageProfile = async (event) => {
        ...
        axios.post(URL + 'UploadPhotos/profile', {
            email: emailUser,
            photoUrl: res.data.url
        })
            .then(async (response) => {
            ...
            <div className={styles.sortProfile}>
                ...
                <div className={styles.sortButtonBack} >
                    <Zoom className={styles.zoomBack} >
                        <button className={styles.buttonBack} onClick={handleInputFile}><Pen className={styles.iconButtons} /> </button>
                    </Zoom>
                    <input onChange={HandleImage} type="file" style={{ display: 'none' }} id="file" name="file" />
                </div>
                ...
                <div className={styles.container}>
                    <div className={styles.sortProfilePicture}>
                        ...
                        <Zoom className={styles.zoomProfile} >
                            <button className={styles.buttonBack} onClick={handleInputFileProfile} ><Pen className={styles.iconButtons} /> </button>
                        </Zoom>
                        <input onChange={HandleImageProfile} type="file" style={{ display: 'none' }} id="fileProfile" name="fileProfile" />
                    </div>
                            ...
                            <div className={styles.boxTechonologies}>
                                ...
                                <div className={styles.separateTechno}>
                                    <div className={styles.sortSelect}>
                                        <label className={styles.labelTechnologies}>Add Technologies</label>
                                        <select onChange={handleTechno} className={styles.select} >
                                            {technologies && technologies.map((item, index) => (
                                                <option className={styles.option} key={index} value={item.name} >{item.name}</option>
                                            ))}
                                        </select>
                                        <button onClick={handleAddTechno} className={styles.buttonBack} >Add</button>
                                    </div>
                                    <div className={styles.sortMyTechnologies}>
                                        <p className={styles.titleMyTechnologies}>My technologies</p>
