...
const Posts = () => {
    ...
    const today = dateToday[0].value + dateToday[1].value + dateToday[2].value + dateToday[3].value + dateToday[4].value;
    ...
    let posts = useSelector(state => state.pink.posts);
        ...
        <Fragment>
            <div className={styles.boxPost}>
                <Zoom className={styles.zoom}>
                    <button onClick={handleEdit} className={styles.buttonPoast}>Create a post</button>
                </Zoom>
            </div>
            {edit ?
                null
                    ...
                    <Zoom className={styles.zoom} >
                        ...
                        <form onSubmit={handleSubmitChange} className={styles.form} >
                            <label className={styles.label} >
                                <Subtitles className={styles.iconButtonsForm} />Title
                            </label>
                            <input
                            ...
                            />
                            <label className={styles.label}>
                                <Photo className={styles.iconButtonsForm} />Image
                            </label>
                            <div className={styles.containerSave}>
                                <button onClick={handleInputFile} type="button" className={styles.buttonBookPop} ><Pen className={styles.iconButtons} /></button>
                            </div>
                            <input onChange={handleImage} type="file" style={{ display: 'none' }} id="filePost" name="filePost" />
                            {imagePost ?
                                <div className={styles.sortFileUSer}>
                                    <img className={styles.imageFile} src={imagePost} alt='' className={styles.imageFile} />
                                </div>
                                :
                                null
                            }
                            <label className={styles.label} >
                                <TextDescription className={styles.iconButtonsForm} />Description
                            </label>
                            <textarea
                                onChange={handleInputChange}
                                name='description'
