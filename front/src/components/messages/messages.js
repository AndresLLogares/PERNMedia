...
import NOFRIENDS from "../images/Messages.gif";
  ...
  return (
    <div className={styles.containerMessages}>
      ...
      {friends && friends.length === 0 ? (
        <div className={styles.sortNoFriends}>
          <Zoom className={styles.zoom}>
            <div className={styles.boxNoFriends}>
              <p className={styles.titleFrinds}>No friends to send messages</p>
            </div>
                ...
                friends.map((item, index) => (
                  <Fragment key={index}>
                    {item.request ? null : (
                      <div
                        onClick={() =>
                          handleFriend(item.emailfriend, item.uuid)
                        }
                        className={styles.eachFriend}
                      >
                        <img
              ...
              <Fragment>
                {friendMessage === "" ? null : (
                  <Fragment>
                    {friends &&
                      friends
                        .filter((item) => item.emailfriend === friendMessage)
                        .map((item, key) => (
                          <Fragment>
                            <div className={styles.upMessages}>
                              <img
                                src={!item.image ? noProfile : item.image}
                                alt=""
                                className={styles.imageFriendMessages}
                              />
                              <p className={styles.name}>{item.namefriend}</p>
                            </div>
                                ...
                                <Fragment>
                                  {item.messages.map((item, key) => (
                                    ...
                                    >
                                      <Fragment>
                                        {item.author === user.fullname ? (
                                          <p className={styles.textMeLittle}>
                                            From: Me
                                          </p>
                                        ) : (
                                          <p className={styles.textMeLittle}>
