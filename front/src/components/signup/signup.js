...
const SignUp = () => {
    ...
    const age = calculateAge(new Date(Number(yearDate), Number(monthDate), Number(dayDate)))
        ...
        await axios.post(URL + 'signup', {
            ...
            .then(async (response) => {
                ...
                if (response.data.message !== 'Thanks for registering') {
                    ...
                    setTimeout(() => window.location.href = 'https://pinkbio.netlify.app/', 1000)
                }
            })
    }
            ...
            <div className={styles.sorTforms}>
                ...
                <form onSubmit={handleSubmitChange} className={styles.form}>
                    ...
                    <Zoom className={styles.zoom} >
                        <label
                            className={styles.label} >
                            <Password className={styles.iconsForm} />Repeat Password
                        </label>
                        <input
                            type='password'
                    ...
                    <Zoom className={styles.zoom} >
                        <label
                            className={styles.label} >
                            <CalendarNumber className={styles.iconsForm} />Date of birth
                        </label>
                        <Calendar
                            onChange={setDate}
                    ...
                    <Zoom className={styles.zoom} >
                        <label
                            className={styles.label} >
                            <DriveFileRenameOutline className={styles.iconsForm} />First Name
                        </label>
                        <input
                            type='text'
                            name='name'
                    ...
                    <Zoom className={styles.zoom} >
                        <label
                            className={styles.label} >
                            <DriveFileRenameOutline className={styles.iconsForm} />Last Name
                        </label>
                        <input
                            type='text'
                            name='lastname'
                    ...
                    <Zoom className={styles.zoom} >
                        <div className={styles.sortButton}>
                            <button type='submit' className={styles.button}><FileSignature className={styles.iconsForm} />Sing Up!</button>
                        </div>
                    </Zoom>
                </form>
            </div>
